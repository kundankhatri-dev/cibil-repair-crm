...
// if (basename($_SERVER['PHP_SELF']) === 'delete_registration_code.php') {
...
if (!$id) {
    ob_clean();
    echo json_encode(['success' => false, 'error' => 'Registration code ID is required']);
    exit();
}
...
$result = mysqli_query($conn, "SELECT * FROM registration_codes WHERE id = $id");
...
if ($registrationCode['is_used'] == 1) {
    // Check if any users have this registration code
    $result = mysqli_query($conn, "SELECT COUNT(*) as count FROM users WHERE registration_code = '{$registrationCode['code']}'");
    $count = mysqli_fetch_assoc($result)['count'] ?? 0;
    if ($count > 0) {
        $related[] = ['type' => 'users', 'count' => $count, 'message' => "$count user(s) have used this code"];
    }
}
...
if (!empty($related) && !$force) {
    ...
    echo json_encode([
        'success' => false,
        'error' => 'Cannot delete registration code. Has related records: ' . implode(', ', $relatedMessages) . '. Use force=true to delete anyway.',
        'data' => [
            'registration_code' => [
                'id' => $id,
...
if ($result) {
    ...
    $logDetails = "Deleted registration code: {$registrationCode['code']} (ID: $id, Role: {$registrationCode['role']})";
    ...
    mysqli_query($conn, "INSERT INTO activity_logs (user_id, user_name, action, details, ip_address, created_at) 
                         VALUES ($user_id, '$user_name', 'Deleted registration code', '$logDetails', '$ip', NOW())");
    ...
    echo json_encode([
        ...
        'data' => [
            ...
            'code' => $registrationCode['code'],
            'role' => $registrationCode['role'],
            'was_used' => (bool)$registrationCode['is_used'],
    ...
    echo json_encode([
        'success' => false,
        'error' => 'Failed to delete registration code: ' . mysqli_error($conn)
    ]);
}
