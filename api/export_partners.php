<?php
// ============================================================
// CIBIL REPAIR CRM - Export Partners API (All Formats)
// Endpoint: /api/export_partners.php
// Method: GET
// ============================================================
...
// Search filter
if (!empty($search)) {
    $where[] = "(name LIKE ? OR location LIKE ? OR owner LIKE ? OR phone LIKE ? OR email LIKE ?)";
    $searchWild = "%$search%";
    $params[] = $searchWild;
    $params[] = $searchWild;
    $params[] = $searchWild;
...
if (empty($partners)) {
    ob_clean();
    echo json_encode(['success' => false, 'error' => 'No partners found to export']);
    exit();
}
...
$filename = 'partners_export_' . date('Y-m-d_H-i-s');
ob_clean();
